...
export default function AdminReceptionists() {
  ...
  const goToReceptionistDashboard = (rec) => {
    localStorage.setItem("receptionistId", rec._id);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`http://localhost:5000/api/receptionists/${id}`, {
        method: "DELETE"
      });
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((rec, i) => (
            <motion.div
              ...
              onClick={() => goToReceptionistDashboard(rec)}
              className="rounded-xl border bg-card p-5 shadow-card cursor-pointer hover:shadow-lg transition"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-primary">{rec.email}</p>
                </div>
              </div>
