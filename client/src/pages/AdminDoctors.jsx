...
export default function AdminDoctors() {
  ...
  const goToDoctorDashboard = (doctor) => {
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:5000/api/ui/doctors/${id}`, {
        method: "DELETE"
      });

      setDoctorList((prev) => prev.filter((d) => d._id !== id));
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((doc, i) => (
            <motion.div
              ...
              onClick={() => goToDoctorDashboard(doc)}
